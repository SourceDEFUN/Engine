...
public:
	...
	virtual void AddPlayerToVoiceList( uint64 xPlayer, int iController ) = 0;
	virtual void RemovePlayerFromVoiceList( uint64 xPlayer, int iController ) = 0;

	virtual void GetRemoteTalkers( int *pNumTalkers, uint64 *pRemoteTalkers ) = 0;
	...
	virtual void SetPlaybackPriority( uint64 remoteTalker, int iController, int iAllowPlayback ) = 0;
	virtual void PlayIncomingVoiceData( uint64 xuid, const byte *pbData, unsigned int dwDataSize, const bool *bAudiblePlayers = NULL ) = 0;
