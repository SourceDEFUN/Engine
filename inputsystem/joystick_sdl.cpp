...
void SearchForDevice()
{
	...
	for ( int device_index = 0; device_index < SDL_GetJoysticks(NULL); ++device_index )
	{
...
void CInputSystem::InitializeJoysticks( void )
{
	...
	const int totalSticks = NULL;
        SDL_JoystickID replacement1 = SDL_GetJoysticks(&totalSticks);
...
void CInputSystem::JoystickHotplugAdded( int joystickIndex )
{
	...
	if ( joystickIndex < 0 || joystickIndex >= SDL_GetJoysticks(NULL) )
	{
		return;
	}
	...
	{
		Msg("Detected supported joystick #%i '%s'. Currently active joystick is #%i.\n", joystickId, SDL_GetJoystickNameForID(joystickIndex), activeJoystick);
		return;
	}
