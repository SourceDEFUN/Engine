...
void SDLCALL audioRecordingCallback(void *userdata, SDL_AudioStream *stream, int additional_amount, int total_amount) {
    if (additional_amount > 0) {
        Uint8 *data = SDL_stack_alloc(Uint8, additional_amount);
        if (data) {
            SDL_GetAudioStreamData(stream, data, additional_amount);
            static_cast<VoiceRecord_SDL*>(userdata)->RenderBuffer(reinterpret_cast<char*>(data), additional_amount);
            SDL_stack_free(data);
        }
    }
}
...
bool VoiceRecord_SDL::InitalizeInterfaces()
{
	...
	m_Device = SDL_OpenAudioDevice( NULL, &spec );
