...
void CAudioDeviceSDLAudio::OpenWaveOut( void )
{
	...
	// TODO: let user specify a device
	// !!! FIXME: we can handle quad, 5.1, 7.1, etc here.
	const SDL_AudioSpec spec = { SDL_AUDIO_S16, 2, SOUND_DMA_SPEED };

        // Open an audio stream
        SDL_AudioStream *m_stream = SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &spec, CAudioDeviceSDLAudio::AudioCallbackEntry, this);
	...
	if ( g_pVideo != NULL) 
	{
		g_pVideo->SoundDeviceCommand( VideoSoundDeviceOperation::SET_SDL_PARAMS, NULL, NULL );
...
void SDLCALL CAudioDeviceSDLAudio::AudioCallbackEntry(void *userdata, SDL_AudioStream *stream, int additional_amount, int total_amount)
{
   ...
   if (additional_amount > 0) {
       ...
       if (data) {
            // MyAudioCallback(userdata, data, additional_amount); // Secton: fuck
