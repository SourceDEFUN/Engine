...
void CSDLMgr::SetWindowFullScreen( bool bFullScreen, int nWidth, int nHeight )
{
	...
	if ( bFullScreen )
	{
		if ( SDL_GetDesktopDisplayMode( displayIndex ) != 0 ) //SDL2 , &mode
		{
			Assert( 0 );
			SDL_GetDesktopDisplayMode( 0 ); //SDL2 , &mode
		}
		 ...
		 ( bFullScreen && ( sdl_displayindex_fullscreen.GetInt() != displayIndex ) ) )
	{
		...
		SDL_SetWindowFullscreen( m_Window, bFullScreen ? SDL_GetWindowFullscreenMode( m_Window ) : 0 );
...
void CSDLMgr::DecWindowRefCount()
{
	...
	if ( !m_nWindowRefCount )
	{
		...
		SDL_SetWindowFullscreen(m_Window, false);  // just
		SDL_SetWindowMouseGrab (m_Window, false);  // in case.
...
void CSDLMgr::GetNativeDisplayInfo( int nDisplay, uint &nWidth, uint &nHeight, uint &nRefreshHz )
{
	...
	if ( SDL_GetDesktopDisplayMode( nDisplay ) != 0 ) //SDL2 , &mode
	{
		Assert( 0 );
		SDL_GetDesktopDisplayMode( 0 ); //SDL2 , &mode
	}
...
void CSDLMgr::SetGammaRamp( const uint16 *pRed, const uint16 *pGreen, const uint16 *pBlue )
{
	/*if ( m_Window )
	{
	        // SDL3 TODO: "use a shader or other in-game effect"!
